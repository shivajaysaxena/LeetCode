class Solution {
public:
    bool canReach(vector<int>& source, vector<int>& target, int numBlocked,
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (currArea > maxArea)
                    return true;
                if (new_x >= 0 && new_x < 1e6 && new_y >= 0 && new_y < 1e6 &&
                    block.find({new_x, new_y}) == block.end() &&
    ...
    bool isEscapePossible(vector<vector<int>>& blocked, vector<int>& source,
        ...
        return canReach(source, target, count, block) &&
               canReach(target, source, count, block);
    }
};